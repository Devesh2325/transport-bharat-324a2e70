...
function LoginPage() {
  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2">
      <div className="hidden lg:flex flex-col justify-between p-12 bg-sidebar text-sidebar-foreground [background:var(--gradient-hero),var(--sidebar)]">
        <Link to="/" className="flex items-center gap-2 font-display text-lg font-bold">
          <span className="grid place-items-center size-8 rounded-md bg-[var(--gradient-brand)]"><Truck className="size-4" /></span>
          TransFlow
        </Link>
        <div>
          <h2 className="text-3xl font-display font-bold">Welcome back to your dispatch desk.</h2>
          <p className="mt-3 opacity-80 max-w-md">Pick up where you left off — pending inquiries, open orders, and today's collections are waiting.</p>
        </div>
        <p className="text-xs opacity-60">Multi-tenant TMS · India-first · GST ready</p>
      </div>
      <div className="flex items-center justify-center p-6">
        <div className="w-full max-w-sm">
          <h1 className="text-3xl font-display font-bold">Sign in</h1>
          <p className="text-sm text-muted-foreground mt-1">Access your TransFlow workspace</p>
          ...
          <p className="mt-6 text-center text-sm text-muted-foreground">
            New to TransFlow? <Link to="/signup" className="text-primary font-medium hover:underline">Create an account</Link>
          </p>
        </div>
      </div>
    </div>
