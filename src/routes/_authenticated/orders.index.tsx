...
const empty = {
  party_id: null as string | null, vehicle_number: "", product_id: null as string | null,
...
function OrdersPage() {
  ...
  // Resolve vehicle: upsert by number, return id. Auto-create silently so user just types.
  const resolveVehicleId = async (): Promise<string | null> => {
    const num = form.vehicle_number.trim().toUpperCase();
    if (!num || !company) return null;
    const { data: existing } = await supabase.from("vehicles").select("id").eq("company_id", company.id).eq("number", num).maybeSingle();
    if (existing?.id) return existing.id;
    const { data: created, error } = await supabase.from("vehicles").insert({ company_id: company.id, number: num }).select("id").single();
  ...
  const submit = async () => {
    ...
    const { error } = await supabase.from("orders").insert({
      ...
      party_id: form.party_id, vehicle_id, product_id: form.product_id,
            ...
            <SheetContent className="w-full sm:max-w-xl overflow-y-auto">
              ...
              <div className="grid gap-3 mt-4">
                ...
                <div><Label>Vehicle Number</Label>
                  <Input value={form.vehicle_number} onChange={e => setForm({ ...form, vehicle_number: e.target.value.toUpperCase() })} placeholder="e.g. MH12AB1234" />
                </div>
                ...
                <div>
                  <Label>Advance received from client (₹)</Label>
                  <Input type="number" value={form.advance_amount} onChange={e => setForm({ ...form, advance_amount: e.target.value })} placeholder="Amount the client has paid upfront" />
                  <p className="text-xs text-muted-foreground mt-1">This reduces the client's balance due. For payments to the transporter, use the Payments page.</p>
                </div>
                <div className="grid grid-cols-2 gap-3">
        ...
        {data && (
          <div className="mt-4 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-2">
              <Button onClick={() => onBilty(data.id)}><FileText className="size-4 mr-1" /> {data.bilty_no ? `Open Bilty ${data.bilty_no}` : "Generate Bilty"}</Button>
